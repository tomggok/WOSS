...
- (id)OJBTOJSON
{
    NSDictionary *dict = [DragonJSONReflection OJBTOJSON:self];
    ...
    for (int i = 0; i < methodCount_f; i++)
    {
        ...
        }else if ([att isEqualToString:@"T@\"NSArray\""])
        {
            ...
            for (int i = 0 ; i < [nValue count]; i++)
            {
                ...
                {
                    result = [DragonJSONReflection OJBTOJSON:obj];
                }
        ...
        }else
        {
            if ([value isKindOfClass:[NSArray class]])
            {
                ...
                {
                    id obj = [nValue objectAtIndex:i];
                    NSDictionary *result = [DragonJSONReflection OJBTOJSON:obj];
                    [arrValue addObject:result];
                }
                value = arrValue;
            }else
            {
                value = [DragonJSONReflection OJBTOJSON:value];
            }
//
        }
        if (!value)
