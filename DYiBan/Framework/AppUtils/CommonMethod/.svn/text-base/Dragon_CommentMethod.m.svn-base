...
typedef void(^ReachChangeBlock)(DragonCommentMethod *);
@interface DragonCommentMethod ()
{}
...
+ (DragonCommentMethod *)reachForHost:(NSString *)host
{
    SCNetworkReachabilityRef reachabilityRef = SCNetworkReachabilityCreateWithName(NULL, [host UTF8String]);
    
    return [[DragonCommentMethod alloc] initWithReachability:reachabilityRef];
}

+ (DragonCommentMethod *)reachForAddress:(const struct sockaddr_in *)addr
{
...
{
    DragonCommentMethod *reach = (DragonCommentMethod *)info;
...
+ (NSString *)downloadPath
{
    ...
    NSString *fullPath = [NSString stringWithFormat:@"%@/%@/",[DragonSandbox docPath], kDOWNLOADPATH];
