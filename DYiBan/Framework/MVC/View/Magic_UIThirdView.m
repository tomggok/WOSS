...
#import "Magic_UIThirdView.h"

@interface MagicUIThirdView ()
{
...
@property (nonatomic, assign)MagicUIThirdScrollState scrollState;
@end

@implementation MagicUIThirdView
...
- (void)setScrollState:(MagicUIThirdScrollState)scrollState
{
    _scrollState = scrollState;
    BOOL isTouch = YES;
    if (scrollState != MagicScrollNomal)
    {
        isTouch = NO;
    }else
    {
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    if (_viewType == MagicUINoScroll)
    {
        return NO;
    }
    
    
    if (_viewType == MagicUILeftView)
    {
        if (startTouch.x + noNum < 0)
        {
            return NO;
        }
    }else if (_viewType == MagicUIRightView)
    {
        if (startTouch.x + noNum > 0)
        {
            return NO;
...
- (void)endMoveViewWithX:(CGFloat)xOffset
{
    ...
    if (xOffset == 0)
    {
        self.scrollState = MagicScrollNomal;
    }else if (xOffset >0)
    {
        [_secondView setHidden:NO];
        [_thirdView setHidden:YES];
        self.scrollState = MagicScrollLeft;
    }else if (xOffset < 0)
    {
        [_secondView setHidden:YES];
        [_thirdView setHidden:NO];
        self.scrollState = MagicScrollRight;
    }
