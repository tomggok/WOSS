...
- (void)initSlimeView
{
    ...
    _slimeView.slime.bodyColor = [DragonCommentMethod colorWithHex:@"009cd5"];
...
- (void)setSlimeViewRefreshHeight:(DragonSlimeRefreshHeightType)refreshType
{
    if (refreshType == DragonReHeightNomal)
    {
        _slimeView.slime.refreshHeight = 0;
    }else if (refreshType == DragonReHeightMiddle)
    {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_tableViewType == DTableViewSlime)
    {
        ...
        if (scrollView.contentOffset.y > scrollerBeginY)
        {
            ...
            {
                [self sendViewSignal:[DragonUITableView TAbLEVIEWSCROLLUP] withObject:dict];
            }
        ...
        }else if (scrollView.contentOffset.y < scrollerBeginY)//向下
        {
            ...
            if (count > 20 && !isScrollMoving)
            {
                [self sendViewSignal:[DragonUITableView TAbLEVIEWSCROLLDOWN] withObject:dict];
            }
        }
