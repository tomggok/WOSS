...
@interface MagicUITabBarView ()
{
    BOOL isFirst;//第一次近来
    
    CGFloat _barHeight;//bar的高度
    
//    MagicUITimeBlurView *timeBlur;//虚化view
}
@property (nonatomic, retain)UIView *transitionView;
@end

@implementation MagicUITabBarView
...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr reduceHeight:(CGFloat)reduceHeight barHeight:(CGFloat)barHeight withClass:(Class)clazz
{
    ...
    if (self != nil)
    {
        ...
        _tabBar = [(MagicUITabBar *)[MagicRuntime allocByClass:clazz] initWithFrame:CGRectMake(0, tabBarY, CGRectGetWidth(self.bounds), _barHeight) buttonImages:arr];
...
//        [self observeNotification:[MagicViewController NOTIFICATIONWILLAPPEAR]];
...
{
    MagicViewSignal *singal = [self sendViewSignal:[MagicUITabBarView TABBARSHOULDSELECT] withObject:[_viewControllers objectAtIndex:index]];
...
//    }
    ...
    MagicViewController *selectedVC = [_viewControllers objectAtIndex:index];
    ...
    for (int i= 0; i < [_viewControllers count]; i++)
    {
        ...
        [[(MagicViewController *)[_viewControllers objectAtIndex:i] view] setUserInteractionEnabled:NO];
    }
    
    [self sendViewSignal:[MagicUITabBarView TABBARDIDSELCT] withObject:selectedVC];
}

- (void)handleViewSignal_MagicUITabBar:(MagicViewSignal *)signal
{
    NSNumber *num = (NSNumber *)[signal object];
    if ([signal is:[MagicUITabBar TABBARBUTTON]])
    {
...
{
    MagicViewController *vc = notification.object;
