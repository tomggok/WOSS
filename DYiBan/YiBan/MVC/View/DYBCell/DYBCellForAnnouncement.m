    ...
    if (data) {
        switch (self.i_type) {
            case 0://公告
            {
                ...
                if (!_lb_time) {
                    _lb_time=[[DragonUILabel alloc]initWithFrame:CGRectMake(self.frame.size.width-90, 5, 0, 0)];
            ...
            case 1://话题
            {
                ...
                if (!_lb_time) {
                    _lb_time=[[DragonUILabel alloc]initWithFrame:CGRectMake(CGRectGetWidth(self.frame)-90, 5, 0, 0)];
                    _lb_time.backgroundColor=[UIColor clearColor];
                    _lb_time.textAlignment=NSTextAlignmentLeft;
                    _lb_time.font=[DYBShareinstaceDelegate DYBFoutStyle:11];
                    //            _lb_time._constrainedSize=CGSizeMake(screenShows.size.width-20, 100);
