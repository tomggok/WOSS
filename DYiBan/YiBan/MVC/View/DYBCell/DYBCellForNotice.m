...
//            }
        ...
        if (!_lb_time) {
            ...
            _lb_time.text=[NSString stringWithFormat:@"%@ 发布了通知",[NSString transFormTimeStamp:model.time]];
            
            [_lb_time setNeedCoretext:NO];
            _lb_time.textColor=(model.view==0/*未读*/)?(ColorRed):(ColorGray);
        ...
        if (!_lb_newContent) {
            ...
            _lb_newContent.textColor=(model.view==0/*未读*/)?(ColorBlack):(ColorGray);
