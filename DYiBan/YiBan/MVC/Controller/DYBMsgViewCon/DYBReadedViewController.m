...
@interface DYBReadedViewController ()
{
...
@implementation DYBReadedViewController

@synthesize muA_readedUser=_muA_readedUser;
...
- (void)handleViewSignal_DragonViewController:(DragonViewSignal *)signal
{
    ...
    if ([signal is:DragonViewController.CREATE_VIEWS]) {
        ...
        _tbv.muA_singelSectionData=_muA_readedUser;
        
//        {//HTTP请求
//            [self.view setUserInteractionEnabled:NO];
//            DragonRequest *request = [DYBHttpMethod user_birthday:@"0" limit:[NSString stringWithFormat:@"%d",_tbv.i_pageNums ]isAlert:YES receive:self];
//            [request setTag:1];
//            
//            if (!request) {//无网路
...
-(void)creatTbv{
    if (!_tbv) {
        _tbv = [[DragonUITableView alloc] initWithFrame:CGRectMake(0, self.headHeight, CGRectGetWidth(self.view.bounds), CGRectGetHeight(self.view.bounds)-self.headHeight) isNeedUpdate:YES];
        _tbv._cellH=65;
...
- (void)handleViewSignal_DragonUITableView:(DragonViewSignal *)signal
{
    ...
    else if ([signal is:[DragonUITableView TABLEHEIGHTFORROW]])//heightForRowAtIndexPath  暂时把每个cell保存,后期有时间优化为只保存高度,返回cell时再异步计算cell的视图,目前刷新后所有cell的view都要重新创建
    {
        ...
        if(indexPath.row==((([tableView isOneSection])?(tableView._muA_differHeightCellView):(arr_curSectionForCell)).count/*只创建没计算过的cell*/) || !tableView._muA_differHeightCellView || ((([tableView isOneSection])?(tableView._muA_differHeightCellView):(arr_curSectionForCell)).count/*只创建没计算过的cell*/)==0)
        {
            ...
            DYBCellForReadedUser *cell = [[[DYBCellForReadedUser alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellName] autorelease];
            ...
            NSNumber *s = [NSNumber numberWithInteger:((DYBCellForReadedUser *)[(([tableView isOneSection])?(tableView._muA_differHeightCellView):(arr_curSectionForCell)) objectAtIndex:indexPath.row]).frame.size.height];
            [signal setReturnValue:s];
        }
    ...
    }else if ([signal is:[DragonUITableView TABLEDIDSELECT]])//选中cell
    {
        ...
        {
            notice *model=[tableview.muA_singelSectionData objectAtIndex:indexPath.row];
            
            DYBPersonalHomePageViewController *vc = [[DYBPersonalHomePageViewController alloc] init];
            
            vc.d_model=[NSMutableDictionary dictionaryWithObjectsAndKeys:model.user.name,@"name",model.user.id,@"userid", nil];
