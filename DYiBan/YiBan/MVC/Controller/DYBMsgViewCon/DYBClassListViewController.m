...
- (void)handleViewSignal_DragonViewController:(DragonViewSignal *)signal
{
    ...
    if ([signal is:DragonViewController.CREATE_VIEWS]) {
        ...
        switch (_type) {
            ...
            case 1://发通知选择通知范围
            {//HTTP请求已加入的班级列表
                DragonRequest *request = [DYBHttpMethod eclass_managelist:SHARED.curUser.userid isAlert:YES receive:self];
                [request setTag:3];
            }
            default:
                break;
        ...
        switch (_type) {
            ...
            case 1:
            {
                if (!_bt_ok) {//
                    ...
                    _bt_ok = [[DragonUIButton alloc] initWithFrame:CGRectMake(self.headview.frame.size.width-img.size.width/2, 0, img.size.width/2, img.size.height/2)];
...
-(void)creatTbv{
    if (!_tbv) {
        _tbv = [[DragonUITableView alloc] initWithFrame:CGRectMake(0, self.headHeight, self.view.frame.size.width, CGRectGetHeight(self.view.bounds)-self.headHeight) isNeedUpdate:YES];
        _tbv._cellH=50;
...
- (void)handleViewSignal_UIView:(DragonViewSignal *)signal
{
    if ([signal is:[UIView TAP]]) {
        ...
        {//HTTP请求,班级活跃度
            ...
            DragonRequest *request = [DYBHttpMethod eclass_active:((eclass *)[_tbv.muA_singelSectionData objectAtIndex:index.row]).id isAlert:YES receive:self];
            [request setTag:2];
//            if (!request) {//无网路
