...
- (void)handleViewSignal_DragonViewController:(DragonViewSignal *)signal
{
    ...
    if ([signal is:DragonViewController.CREATE_VIEWS]) {
        ...
        {//HTTP请求
            ...
            DragonRequest *request = [DYBHttpMethod status_notice_id:_str_noticeID type:@"8" page:@"1" num:@"10" isAlert:YES receive:self];
            [request setTag:1];
...
-(void)creatTbv{
    if (!_tbv) {
        _tbv = [[DragonUITableView alloc] initWithFrame:CGRectMake(0, self.headHeight, CGRectGetWidth(self.view.bounds), CGRectGetHeight(self.view.bounds)-self.headHeight-50) isNeedUpdate:YES];
        //        _tbv._cellH=92;
    ...
    if (/*[SHARED.curUser.usertype intValue]==1 [[NSString stringWithFormat:@"%d",_status.status.userid] isEqualToString:SHARED.curUser.userid]*/ _b_isInstructorInNoticeClass) {//辅导员
        {
            ...
            DragonUIButton *_bt_refresh = [[DragonUIButton alloc] initWithFrame:CGRectMake(_v_tabbar.frame.size.width-75, 0,75, CGRectGetHeight(_v_tabbar.frame))];
            _bt_refresh.tag=1;
            _bt_refresh.backgroundColor=[UIColor clearColor];//self.headview.backgroundColor;
            //            _bt_DropDown.alpha=0.9;
            [_bt_refresh addSignal:[DragonUIButton TOUCH_UP_INSIDE] forControlEvents:UIControlEventTouchUpInside object:nil];
        ...
        if (!_bt_IKnow) {
            //                UIImage *img= [UIImage imageNamed:@"msg_del"];
            _bt_IKnow = [[DragonUIButton alloc] initWithFrame:CGRectMake(0, 0,100, CGRectGetHeight(_v_tabbar.frame))];
            _bt_IKnow.tag=2;
            _bt_IKnow.backgroundColor=[UIColor clearColor];//self.headview.backgroundColor;
            //            _bt_DropDown.alpha=0.9;
            [_bt_IKnow addSignal:[DragonUIButton TOUCH_UP_INSIDE] forControlEvents:UIControlEventTouchUpInside object:nil];
            //                [_bt_delete setImage:img forState:UIControlStateNormal];
            //            [_bt_sendNotice setBackgroundImage:[UIImage imageNamed:@"btn_mainmenu_hilight"] forState:UIControlStateHighlighted];
...
//                        [_tbv.footerView changeState:VIEWTYPEFOOTER];
                ...
                case 2://我知道了
                {
                    {//HTTP请求
                        ...
                        DragonRequest *request = [DYBHttpMethod status_feedaction_id:_str_noticeID action:@"3" type:@"1" isAlert:YES receive:self];
                        [request setTag:2];
