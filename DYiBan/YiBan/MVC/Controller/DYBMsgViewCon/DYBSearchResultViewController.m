...
- (void)handleViewSignal_DragonViewController:(DragonViewSignal *)signal
{
    ...
    if ([signal is:DragonViewController.CREATE_VIEWS]) {
        ...
        {//HTTP请求
            ...
            DragonRequest *request = [DYBHttpMethod search_user:_str_searchContent num:@"10" page:1 isAlert:YES receive:self];
            [request setTag:1];
...
- (void)handleViewSignal_DragonUITableView:(DragonViewSignal *)signal
{
    ...
    else if ([signal is:[DragonUITableView TAbLEVIEWLODATA]])//加载更多
    {
        ...
        {//HTTP请求
            [self.view setUserInteractionEnabled:NO];
            DragonRequest *request = [DYBHttpMethod search_user:_str_searchContent num:@"10" page:++_tbv._page isAlert:YES receive:self];
            [request setTag:2];
            if (!request) {//无网路
    ...
    else if ([signal is:[DragonUITableView TABLEVIEWUPDATA]])//刷新
    {
        ...
        {//HTTP请求
            [self.view setUserInteractionEnabled:NO];
            DragonRequest *request = [DYBHttpMethod search_user:_str_searchContent num:@"10" page:_tbv._page=1 isAlert:YES receive:self];
            [request setTag:1];
            if (!request) {//无网路
                [tableView reloadData:NO];
            }
