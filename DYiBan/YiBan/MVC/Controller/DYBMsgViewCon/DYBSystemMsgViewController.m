...
- (void)handleViewSignal_DragonViewController:(DragonViewSignal *)signal
{
    ...
    if ([signal is:DragonViewController.CREATE_VIEWS]) {
        ...
        {//HTTP请求
            ...
            DragonRequest *request = [DYBHttpMethod message_friendreqlist_yaoqing:1 pageNum:10 isAlert:YES receive:self];
            [request setTag:1];
    ...
    if ([signal is:[DragonUIButton TOUCH_UP_INSIDE]]) {
        ...
        if (bt)
        {
            switch (bt.tag) {
                ...
                {//HTTP请求
                    [self.view setUserInteractionEnabled:NO];
                    DragonRequest *request = [DYBHttpMethod message_applyfriend:[d objectForKey:@"userId"] op:[NSString stringWithFormat:@"%d",bt.tag] isAlert:YES receive:self];
                    [request setTag:3];
...
- (void)handleViewSignal_DragonUITableView:(DragonViewSignal *)signal
{
    ...
    else if ([signal is:[DragonUITableView TAbLEVIEWLODATA]])//加载更多
    {
        ...
        {//HTTP请求
            [self.view setUserInteractionEnabled:NO];
            DragonRequest *request = [DYBHttpMethod message_friendreqlist_yaoqing:++tableView._page pageNum:10 isAlert:YES receive:self];
            [request setTag:2];
    ...
    else if ([signal is:[DragonUITableView TABLEVIEWUPDATA]])//刷新
    {
        ...
        {//HTTP请求
            [self.view setUserInteractionEnabled:NO];
            DragonRequest *request = [DYBHttpMethod message_friendreqlist_yaoqing:(tableView._page=1) pageNum:10 isAlert:YES receive:self];
            [request setTag:1];
            ...
            case 3://同意|忽略
            {
                ...
                if ([response response] ==khttpsucceedCode)
                {
                    if ([[response.data objectForKey:@"apply"] isEqualToString:@"1"]) {//成功
                        ...
                        {//HTTP请求,刷新列表
                            [self.view setUserInteractionEnabled:NO];
                            DragonRequest *request = [DYBHttpMethod message_friendreqlist_yaoqing:(_tbv._page=1) pageNum:10 isAlert:YES receive:self];
                            [request setTag:1];
...
- (void)handleViewSignal_DYBSystemMsgViewController:(DragonViewSignal *)signal{
